...
//THis function is used to upload driver's license
function addlicense(){
	$name=$_POST['name'];   
$email=$_POST['email'];
   // echo $b;
   ...
   // $_FILES['files']['tmp_name'][0];
        $file_name1= $_FILES['license']['name'];
        $ext = pathinfo($file_name1, PATHINFO_EXTENSION);
        $file_name=$type.$name.".".$ext;
...
else{    //Added file size limit
         if($file_size > 2097152){
            $errors='File size must be less than 2 MB';
        }  
         if(empty($errors)==true){    //Inserting info into database for easy retrival
          $query= $this->pdo->prepare("INSERT into upload_data (NAME,EMAIL,FILE_NAME,FILE_SIZE,FILE_TYPE,date1,file_ext,type) VALUES('$name','$email','$file_name',$file_size,'$file_type','$date12','$ext','$type')");
  $query->execute();
...
//This function will add the  photo and update its details in the database
function addphoto(){
	$name=$_POST['name'];   
$email=$_POST['email'];
   // echo $b;
...
function addDriver()
{
	...
	$query->execute();
	$query1= $this->pdo->prepare("insert into users values ('','$name', '', '$email', '$no', 'driver', '')");
	$query1->execute();
	$rowsadded = $query->rowCount();
	$rowsadded1 = $query1->rowCount();

	if ($rowsadded == 1 && $rowsadded1 == 1)
	{	
...
function getDriverGroups(){
	...
	$query = $this->pdo->prepare("SELECT group_id, group_name FROM groups");
	$query->execute();
	print"<option value='0'>Any</option>";	
...
function addGroup()
{
	...
	$query= $this->pdo->prepare("INSERT INTO groups VALUES ('','$group_name')");
	$query->execute();
	...
	print "<div class='alert alert-primary alert-white-alt rounded'>
								<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
								<div class='icon'><i class='fa fa-check'></i></div>
								<strong>Success! </strong>New Group added!</div>";
	}
	else
	{
	print "<div class='alert alert-danger alert-white rounded'>
...
echo "hi";
	$query0=$this->pdo->prepare("SELECT * FROM admin where username = '$user' ");
	$query0->execute();
	$row=$query0->fetch(PDO::FETCH_OBJ);
	$emaili=$row['user_email'];
	$pass  =  rand(1000000,9999999);//FETCHING PASS
					$username= $_POST['username'];

					$query=$this->pdo->prepare("UPDATE users SET password='$pass' where user_name = '$username'");
	$query->execute();
	$count =$query->rowCount();
	if ($count == 1)
...
if(!$mail->send()) { 
	print "<h3>An error occured while sending the mail.</h3>";
    
}
			 else {
   
   print "<h3>An email has been sent to your email address.Please use the new password provided there.</h3>";
}}
else{
...
function forgotapp(){
	$query0=$this->pdo->prepare("SELECT * FROM users where user_email = '$email' ");
	$query0->execute();
	$row=$query0->fetch(PDO::FETCH_OBJ);
	$emaili=$row['user_email'];
	...
	$pass  =  rand(1000000,9999999);//FETCHING PASS
					...
					$query=$this->pdo->prepare("UPDATE users SET password='$pass' where user_email = '$emaili'");
	$query->execute();
	$count =$query->rowCount();
	if ($count == 1)
	{	$rows=$query->fetch(PDO::FETCH_OBJ);
...
$mail->SMTPAuth = true;                               // Enable SMTP authentication
$mail->Username = 'cyclonemail1@gmail.com';                 // SMTP username
$mail->Password = 'cyclonemail11';                           // SMTP password
$mail->SMTPSecure = 'ssl';                            // Enable TLS encryption, `ssl` also accepted
$mail->Port = 465;                                    // TCP port to connect to

$mail->setFrom('cyclonemail1@gmail.com', 'Cyclone Pty');
$mail->addAddress($emaili,$username);     // Add a recipient
...
echo "Could not find any records!";
}
...
function mapOffers()
{
	...
	while ($maps=$query->fetch(PDO::FETCH_OBJ))
	{
		...
		echo ("addMarkerRed($maps->latitude, $maps->longitude,'$name<br/>$no' )\n;");
	}
...
{
$expiry = date("Y-m-d");	
/*$query=$this->pdo->prepare("SELECT * FROM users JOIN groups ON users.group_id = groups.group_id where category ='driver' ORDER BY user_name");*/
	
	$query=$this->pdo->prepare("SELECT * FROM users where category ='driver' ORDER BY user_name");
$query->execute();

while($offers=$query->fetch(PDO::FETCH_OBJ))
	{	$query1=$this->pdo->prepare("SELECT * FROM upload_data where name ='$offers->user_name' AND type='photo'");
		$query1->execute();
		$offers1=$query1->fetch(PDO::FETCH_OBJ);	
		$query2=$this->pdo->prepare("SELECT * FROM upload_data where name ='$offers->user_name' AND type='license'");
		$query2->execute();
		$offers2=$query2->fetch(PDO::FETCH_OBJ);		
		print "<tr id='user_id_".$offers->user_id."' class='odd gradeX'>
					<td id='user_name_".$offers->user_id."'>$offers->user_name</td>
					<td id='user_email_".$offers->user_id."'>$offers->user_email</td>
					<td id='user_num_".$offers->user_id."'>$offers->user_num</td>
					<td>
						<p id='group_name_".$offers->user_id."'></p>
						<select id='group_id_".$offers->user_id."_edit' name='group_id_".$offers->user_id."_edit' class='form-control' style='display: none'>";
							$this->getAllGroups($offers->group_id);
						print "</select>
					</td>
					<td>
						<img id='button_edit_".$offers->user_id."' src='../admin/images/Edit.png' style='display: block' alt='Edit' height='18' width='18' onclick='edit_row($offers->user_id)'>
						<img id='button_save_".$offers->user_id."' src='../admin/images/Save.png' style='display: none' alt='Edit' height='18' width='18' onclick='save_row($offers->user_id)'>
					</td>
					<td>
						<img id='button_delete_".$offers->user_id."' src='../admin/images/Delete.png' style='display: block' alt='Delete' height='18' width='18' onclick='delete_row($offers->user_id)'>
						<img id='button_cancel_".$offers->user_id."' src='../admin/images/Cancel.png' style='display: none' alt='Delete' height='18' width='18' onclick='cancel_row($offers->user_id)'>
					</td>
					<td>
						<a href='../admin/user_data/".$offers2->FILE_NAME."' target='_blank'><img id='license".$offers->user_id."' src='../admin/user_data/".$offers2->FILE_NAME."' alt='license' height='18' width='18' ></a>
						
					</td>		
					<td>			
					<a href='../admin/user_data/".$offers1->FILE_NAME."'><img id='photo".$offers->user_id."' target='_blank' src='../admin/user_data/".$offers1->FILE_NAME."' alt='photo' height='18' width='18' ></a>
					</td>
				</tr>";
...
function getOnlineDrivers(){
	...
	$query = $this->pdo->prepare("SELECT user_id,user_name FROM users JOIN locations ON users.user_name  = locations.name WHERE locations.online=1");
	$query->execute();
	 while ($row=$query->fetch(PDO::FETCH_OBJ))
	{ 	
...
function getOnlineDriversByGroups(){
	...
	if ($group_id != 0)
	{
		$query = $this->pdo->prepare("SELECT user_id,user_name,online FROM users JOIN locations ON users.user_email  = locations.email WHERE locations.online=1 AND users.group_id='$group_id' ORDER BY user_name");
	}
	else
	{
		$query = $this->pdo->prepare("SELECT user_id,user_name,online FROM users JOIN locations ON users.user_email = locations.email  WHERE locations.online=1 ORDER BY user_name");
	}
	...
	while ($row=$query->fetch(PDO::FETCH_OBJ))
	{ 
		if($row->online==1){$back=green;}
		print"<option 	style='background-color:green' value='$row->user_id'>$row->user_name</option>";	
	}
}

function getDriversByGroup($group_id){
	$query = $this->pdo->prepare("SELECT user_id,user_name FROM users JOIN locations ON users.user_email = locations.email WHERE locations.online=1 AND users.group_id='$group_id' ORDER BY user_name");
...
function allgetDriversByGroup($group_id){
	
	$query = $this->pdo->prepare("SELECT user_id,user_name,online FROM users JOIN locations ON users.user_email = locations.email WHERE users.group_id='$group_id' ORDER BY user_name");
...
function getAllDrivers(){
	...
	$query = $this->pdo->prepare("SELECT user_id, user_name FROM users WHERE category='driver' ORDER BY user_name");
	$query->execute();
	 while ($row=$query->fetch(PDO::FETCH_OBJ))
	{ 	
    print"<option value='$row->user_id'>$row->user_name</option>";
...
function getAllGroups($group_id){
	$query = $this->pdo->prepare("SELECT group_id, group_name FROM groups ORDER BY group_name");
	$query->execute();
	 while ($row=$query->fetch(PDO::FETCH_OBJ))
	{ 	
		if ($group_id!= "" && $row->group_id==$group_id)	
			print"<option value='$row->group_id' selected>$row->group_name </option>";	
		else
			print"<option value='$row->group_id'>$row->group_name </option>";	
	}
}

function updateRequest($id, $accept, $driver_id, $driver_email, $driver_name)
{
	$query = $this->pdo->prepare("UPDATE texirequest SET accept='$accept', driver_id='$driver_id', driver_email='$driver_email', driver_name='$driver_name' where id = '$id'");
	$query->execute();
	return true;
}
...
{	$rate=1;
	...
	{
		$date = new DateTime($_POST['booking_time']); 
		$booking_time = $date->format('Y-m-d H:i:s');
	}
	else
	{
		$booking_time = $timedate;
	}

	$types=$_POST['types'];
	
	if($types == 'VehicleDriver' || $types=='Self-Drive'){
	...
	$destinationlong = $_POST['destinationlong'];
		
	$query= $this->pdo->prepare("insert into texirequest(driver_id, name,phone,location,droplocation,vehicle_type,timedate,num_passengers,group_id,booking_time,types,latitude,longitude,drop_latitude,drop_longitude,rate,position) values ('$driver_id','$name',$no, '$location', '$destination', '$vehicle_type','$timedate','$num_passengers','$group_id','$booking_time','$types','$locationlat','$locationlong','$destinationlat','$destinationlong','$rate','$position')");
	$query->execute();
	...
	print "<div class='alert alert-primary alert-white-alt rounded'>
								<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
								<div class='icon'><i class='fa fa-check'></i></div>
								<strong>Success! </strong>New Booking added!</div>";
	}
	else
	{
	print "<div class='alert alert-danger alert-white rounded'>
...
function approveRequests()
{
	...
	$query=$this->pdo->prepare("SELECT texirequest.id, accept, name, location, droplocation, phone, booking_time, 'types', num_passengers, driver_id, user_name, texirequest.group_id, group_name FROM texirequest LEFT JOIN groups ON texirequest.group_id = groups.group_id LEFT JOIN users ON texirequest.driver_id = users.user_id ORDER BY id DESC");
	$query->execute();
	...
	{
		$id=$offers->id;	
		$arr = array("Not Accepted Yet", "Accepted", "Completed", "Cancelled");
		
		if($offers->accept == 0)
		{
			$arr = array("Not Accepted Yet", "Accepted", "Cancelled");
		}
		
		echo "<tr class='odd gradeX'>
					<td>$num</td>
					<td>$offers->name</td>
					<td id='start".$offers->id."'>$offers->location</td>
					<td id='end".$offers->id."'>$offers->droplocation</td>
					<td>$offers->phone</td>
					<td>".date("d-m-Y H:i:s",strtotime($offers->booking_time))."</td>
					<td>$offers->types</td>
					<td>$offers->num_passengers</td>
					<td><p id='user_name_".$offers->id."'>$offers->user_name</p>
						<select id='user_id_".$offers->id."_edit' name='user_id_".$offers->id."_edit' class='form-control' style='display: none'>";

		$this->allgetDriversByGroup($offers->group_id);
		if ($offers->accept == 0)
		{
			print "</select>
						<img id='button_edit_name_".$offers->id."' src='../admin/images/Edit.png' style='display: block' alt='Edit' height='18' width='18' onclick='edit_row_name($offers->id)'>
						</td>";
		}

		print "<td><p id='group_name_".$offers->id."'>$offers->group_name</p>
						<select id='group_id_".$offers->id."_edit' name='group_id_".$offers->id."_edit' class='form-control' style='display: none'>";
		if ($offers->accept == 0)
		{
		$this->getAllGroups($offers->group_id);
		print "</select>
					<img id='button_edit_group_".$offers->id."' src='../admin/images/Edit.png' style='display: block' alt='Edit' height='18' width='18' onclick='edit_row_group($offers->id)'>
					</td>";
		}

		print "<td>";
					
		if ($offers->accept == 2 || $offers->accept == 3)
		{
			echo "<select onfocus='this.oldvalue = this.value;' onchange='func($id,this)' disabled>";
		}
		else
		{
			echo "<select onfocus='this.oldvalue = this.value;' onchange='func($id,this)' >";
		}
					
		foreach ($arr as $key=>$value)
		{
			if ($key == $offers->accept)
			{							
				echo "<option value=".$offers->accept." selected='selected'>".$value."</option>";									
			}
			else
			{
		...
		{
			echo "<td>
				<img id='button_save_name_".$offers->id."' src='../admin/images/Save.png' style='display: none' alt='Edit' height='18' width='18' onclick='save_row_name($offers->id)'>
				<img id='button_save_group_".$offers->id."' src='../admin/images/Save.png' style='display: none' alt='Edit' height='18' width='18' onclick='save_row_group($offers->id)'>
		      	</td>
		      	<td>
		      		<img id='button_cancel_name_".$offers->id."' src='../admin/images/Cancel.png' style='display: none' alt='Delete' height='18' width='18' onclick='cancel_row_name($offers->id)'>
		      		<img id='button_cancel_group_".$offers->id."' src='../admin/images/Cancel.png' style='display: none' alt='Delete' height='18' width='18' onclick='cancel_row_group($offers->id)'>
						</td>";
		}
		else
		{
			echo "<td><td>
						<img id='map".$offers->id."' src='../admin/images/map.png' style='display: block' alt='Ma' pheight='18' width='18' onclick='popupCenter($offers->id, 450, 450)'>
					</td>";
					//echo"<td>&nbsp;</td>";
		}
...
function updateStatus($val,$id){
	$query=$this->pdo->prepare("update texirequest set accept=".$val." where id=".$id." ");
$query->execute();
}	

function addMessage($user_id,$fromadmin,$message,$datetime){
	$query=$this->pdo->prepare("INSERT INTO chat (user_id, fromadmin, message, datetime) VALUES ('".$user_id."', '".$fromadmin."', '".$message."', '".$datetime."') ");
	$query->execute();
}

function getAllMessages(){
	$query=$this->pdo->prepare("SELECT * FROM chat JOIN users ON chat.user_id = users.user_id ORDER BY id DESC");
	$query->execute();
	while ($row=$query->fetch(PDO::FETCH_OBJ)) { 	
	   if ($row->fromadmin == 1) {
	      print "<tr><td>Admin</td><td>$row->user_name</td><td>$row->message</td><td>".date("d-m-Y H:i:s",strtotime($row->datetime))."</td></tr>";
	   }
	   else {
	      print "<tr><td>$row->user_name</td><td>Admin</td><td>$row->message</td><td>".date("d-m-Y H:i:s",strtotime($row->datetime))."</td></tr>";
	   }	
	}
}
...
function func(id,selectVal){
	
	var s;
	var o = selectVal.oldvalue;
	...
	var r = confirm('Are you sure you want to change the state to '+s+'?');
	...
	if (r == false) {
		...
		$.ajax({
                        ...
                        async: false,
                        url: "ajax.php",
                        data: {"id":id, "val":selectVal.value},
